...
// 徽章成长四阶段：
// 0=未开始(不展示) 1=轮廓(学完1章) 2=标志(50%章节) 3=点亮(全部章节) 4=闪耀(考核通过)
// 标识统一使用星星 ★（不区分模块，成长象征）
...
const starSize = computed(() => Math.round(props.size * 0.5))
</script>
...
<template>
  ...
  >
    <text v-if="showIcon" class="star" :style="{ fontSize: starSize + 'px' }">★</text>
    <view v-if="level >= 4" class="mb-check">✓</view>
  </view>
</template>
...
.lv4 {
  border: 4px solid var(--sov-black);
  background: var(--sov-gold);
  box-shadow: 0 0 10px 2px rgba(212, 168, 67, .9);
